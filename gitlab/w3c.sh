...
do
	...
	cd $DIR
	$DIR/vnu-runtime-image/bin/vnu --errors-only --exit-zero-always --skip-non-css --format json $url 2> result.json
    #trace_off
	...
	$DIR/vnu-runtime-image/bin/vnu --errors-only --exit-zero-always --skip-non-svg --format json $url 2> result.json
    #trace_off
	...
	$DIR/vnu-runtime-image/bin/vnu --errors-only --exit-zero-always --skip-non-html --format json $url 2> result.json
    #trace_off
    ...
    #trace_on
    	...
    	cat htmltmp | grep -v "Attribute “loading” not allowed on element" | grep -v "Element “style” not allowed as child of element"
	NEWFOUNDERRORS=`cat htmltmp | grep -v "Attribute “loading” not allowed on element" | grep -v "Element “style” not allowed as child of element" | wc -l`
