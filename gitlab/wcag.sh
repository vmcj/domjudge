...
httrack http://localhost/domjudge/$SCRAPEURL --assume html=text/html -*doc* -*login* -*logout*
rm index.html
...
do
	...
	#if [ "$1" == "public" ]; then
	su domjudge -c "pa11y --runner axe -T $ACCEPTEDERR --ignore page-has-heading-one -E '#menuDefault > a' --reporter json ./$file" | python -m json.tool
        ERR=`su domjudge -c "pa11y --runner axe --ignore page-has-heading-one -T $ACCEPTEDERR -E '#menuDefault > a' --reporter csv ./$file" | wc -l`
	FOUNDERR=$((ERR+FOUNDERR-1)) # Remove header row
	#fi
	for standard in $STANDARDS
	do
		su domjudge -c "pa11y -s $standard -T $ACCEPTEDERR -E 'a > button','#menuDefault > a','html > body > main > div > div > div > div:nth-child(3) > div:nth-child(1) > h3' -i WCAG2AA.Principle1.Guideline1_4.1_4_3.G18.Fail -i WCAG2AAA.Principle1.Guideline1_4.1_4_6.G17.Fail --reporter json ./$file" | python -m json.tool
		ERR=`su domjudge -c "pa11y -s $standard -T $ACCEPTEDERR -E 'a > button','#menuDefault > a','html > body > main > div > div > div > div:nth-child(3) > div:nth-child(1) > h3' -i WCAG2AA.Principle1.Guideline1_4.1_4_3.G18.Fail -i WCAG2AAA.Principle1.Guideline1_4.1_4_6.G17.Fail --reporter csv ./$file" | wc -l`
		FOUNDERR=$((ERR+FOUNDERR-1)) # Remove header row
	done
	section_end $file
done
