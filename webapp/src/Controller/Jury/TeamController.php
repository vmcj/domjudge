...
class TeamController extends BaseController
{
    ...
    public function deleteListAction(Request $request): Response
    {
        ...
        foreach (array_keys($request->request->all()) as $key) {
            ...
            if (!$entity) {
                throw new NotFoundHttpException(sprintf('Team with ID %s not found', $entityId));
            }
            $entitiesToDelete[] = $entity;
        }

        if (count($entitiesToDelete)===0) {
            $this->addFlash('warning', 'No teams selected.'); 
            return $this->redirectToRoute('jury_teams');
        }

        return $this->deleteEntities($request, $this->em, $this->dj, $this->eventLogService, $this->kernel,
                                     $entitiesToDelete, $this->generateUrl('jury_teams'));
    }
