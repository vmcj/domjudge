...
class LanguageController extends BaseController
{
    ...
    /**
     * @Route("/deleteList", name="jury_languages_delete")
     * @IsGranted("ROLE_ADMIN")
     */
    public function deleteListAction(Request $request): Response
    {
        ...
        foreach (array_keys($request->request->all()) as $key) {
            ...
            if (!$entity) {
                throw new NotFoundHttpException(sprintf('Language with ID %s not found', $entityId));
            }
            $entitiesToDelete[] = $entity;
        }

        if (count($entitiesToDelete)===0) {
            $this->addFlash('warning', 'No languages selected.'); 
            return $this->redirectToRoute('jury_languages');
        }

        return $this->deleteEntities($request, $this->em, $this->dj, $this->eventLogService, $this->kernel,
                                     $entitiesToDelete, $this->generateUrl('jury_languages'));
    }
